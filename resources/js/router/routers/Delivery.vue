    ...
    <div class="Delivery cnt p-t">
        ...
        <div class="ProfileSearch">
            ...
            <div class="ProfileSearchContent">
                ...
                <OrderItem v-for="item in filtersOrder" :getOredes="getOredes" :key="item.id" :item="item" />
            </div>
        </div>
    </div>
</template>
    ...
    computed: {
        filtersOrder() {
            return this.OrderCarts.filter((item) => {
                ...
                if (id == 7) {
                    item.json_cart.cart = item.json_cart.cart.filter(cart => {
                        cart.options = cart.options.filter(op => {
                            return op.delivery == true
                        })
